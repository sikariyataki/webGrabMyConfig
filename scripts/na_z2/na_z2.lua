...
local function ZapToDate(zapdate)
	...
	if a==nil then 
		print("Invalid Zap2It date: "..zapdate)
		end
	return date
	end
...
local ListingsURL="http://datadirect.webservices.zap2it.com/tvlistings/xtvdService"
...
local function DownloadData(grabber,from,to,cachefile,cacheage)
	...
	if (err<400)and(err~=-1) then
		...
		elseif err==401 then print("Zap2it Authentication error, check your password in the grabber settings")
		else print("Error downloading Zap2It data, error code: "..err)
		end
	return err
	end
...
local function Pass2Func(grabber,channel,maxdays)
	...
	for i,prog in ipairs(station) do
		...
		programme.PreviouslyShown=prog.Repeat
		if prog.Stereo then
	...
	for lup in string.gfind(lups,"(.-);") do
		...
		if lineup then
			...
			for i,map in ipairs(lineup.Map) do
				...
				chan.Name={}
				table.insert(chan.Name,zapdata.Stations[map.Station].Name)
				table.insert(chan.Name,zapdata.Stations[map.Station].CallSign)
				table.insert(chan.Name,zapdata.Stations[map.Station].Affiliate)
				table.insert(chan.Name,zapdata.Stations[map.Station].fccChannelNumber)
...
Grabbers.NA_Z2={
	...
	Version = "1.00.00",
	Author = "Alan Birtles",
	InfoURL = "http://www.birtles.org.uk/xmltv/wiki/index.php/Zap2it",
	SourceURL = "http://labs.zap2it.com/",
